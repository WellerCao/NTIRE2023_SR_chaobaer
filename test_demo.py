...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        from models.team14_NAFFinal import Final
        name, data_range = f"{model_id:02}_NAFFinal", 1.0
        model_path = os.path.join('model_zoo', 'team14_NAFFinal.pth')
        model = Final()
        model.load_state_dict(torch.load(model_path), strict=True)
    else:
...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    for i, (img_lr, img_hr) in enumerate(data_path):
        ...
        img_lr = util.imread_uint(img_lr, n_channels=6)
...
def main(args):
    ...
    if True:
        ...
        input_dim = (6, 256, 256)  # set the input dimension
